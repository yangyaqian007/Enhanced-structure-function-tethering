...
% network. Adjusted node-wise $R^2$ estimate locael structure-function
% correspondence.
rsq = zeros(N, 1); % node-wise $R^2$ vector
...
%% Fig2-2 regional zscore

zscore1=(R_R1-mean(ci_perm1,2))./std(ci_perm1,[],2);
p = normcdf(zscore1);
p1=zeros(size(p));
p1(zscore1>0)=1-p(zscore1>0);
p1(zscore1<0)=p(zscore1<0);
[h, ~, ~, p1]=fdr_bh(p1); 
figure;
bar(zscore1,'FaceColor','flat','LineStyle','none');
...
%% Reference
% [1] J. M. Huntenburg,et al., Large-scale gradients in human cortical organization. Trends Cogn. Sci. 22, 21â€?1 (2018).
%
